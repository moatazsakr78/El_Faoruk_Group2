...
const CACHE_TTL = 30000; // مدة صلاحية التخزين المؤقت بالمللي ثانية (30 ثانية)
...
function getCachedData<T>(key: string): T | null {
  ...
  if (now - entry.timestamp > CACHE_TTL) {
    // البيانات قديمة، إزالتها من التخزين المؤقت
...
// إنشاء عميل Supabase مع تعديل لمنع الاستعلامات المتكررة
const baseClient = createClient(supabaseUrl, supabaseKey, supabaseOptions);

// إنشاء نسخة معدلة من العميل مع تخزين مؤقت
export const supabase = {
  ...baseClient,
  // تجاوز دالة from لإضافة التخزين المؤقت
  from: (table: string) => {
    const originalFrom = baseClient.from(table);
    
    // تجاوز دالة select لإضافة التخزين المؤقت
    ...
    originalFrom.select = function(columns?: string) {
      ...
      // تجاوز دالة eq لإضافة التخزين المؤقت
      const originalEq = selectQuery.eq;
      selectQuery.eq = function(column: string, value: any) {
        const eqQuery = originalEq.call(this, column, value);
        
        // تجاوز دالة single لإضافة التخزين المؤقت
        const originalSingle = eqQuery.single;
        eqQuery.single = async function() {
          // إنشاء مفتاح للتخزين المؤقت
          const cacheKey = `${table}:${columns || '*'}:${column}:${value}:single`;
          ...
          // إذا لم تكن هناك بيانات مخزنة مؤقتًا، قم بإجراء الاستعلام
  ...
  // استخدام الدوال الأصلية للقنوات والمصادقة
...
export async function forceRefreshFromServer() {
  ...
  try {
    ...
    const appModels = productsData.map(item => {
